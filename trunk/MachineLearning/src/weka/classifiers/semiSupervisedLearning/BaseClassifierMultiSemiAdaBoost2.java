...
/**
 * 主要为了配合半监督学习用看，看是否真的提高了基分类器的效果，使采用的标记数据相同
 *2009-01-03
 * @author huangdongshan
 */
public class BaseClassifierMultiSemiAdaBoost2 extends RandomizableIteratedSingleClassifierEnhancer
...
//
    ...
    protected void buildClassifierWithWeights(Instances traindata) throws Exception {
        ...
        for (m_NumIterationsPerformed = 0; m_NumIterationsPerformed < m_Classifiers.length;
            ...
            //建立该轮训练分类器
            m_Classifiers[m_NumIterationsPerformed].buildClassifier(labledDataSet);
            //对训练池中的数据进行评价，当m_NumIterationsPerformed=0
            //trainPool中的数据都是实际的label，没有伪label
            evalTrainPool = new Evaluation(labledDataSet);
            evalTrainPool.evaluateModel(m_Classifiers[m_NumIterationsPerformed], labledDataSet);
            ...
            //这个地方应该对分类正确和错误都加权
            setWeights(labledDataSet, reweightcorrect, reweightincorrect);
