 ...
 */
public class MultiSemiAdaBoost2_2_1 extends RandomizableIteratedSingleClassifierEnhancer
    ...
    private int m_kNN = 1;
    /*标记数据和未标记数据的权重比率*/
    private double m_lurate = 2.0;
    private double m_unlabebledWeights = 1.0;
    /*分类错了的标记数据与伪标记数据的权重比例*/
    private double m_luincorrectRate = 2.0;
...
//    }
     ...
     * @param value
     */
    public void setLuincorrectRate(double value) {
        m_luincorrectRate = value;
    }

    public double getLuincorrectRate() {
        return m_luincorrectRate;
    }
     ...
     */
    public void setLabebledUnlabledRate(double ulrate) {
        m_lurate = ulrate;
    }

    public double getLabebledUnlabledRate() {
        return m_lurate;
    }
...
//        knn.setKNN(m_kNN);
        ...
        //设置labeled data的权重
        Enumeration labled = trainPool.enumerateInstances();
        while (labled.hasMoreElements()) {
            Instance l = (Instance) labled.nextElement();
            l.setWeight(m_unlabebledWeights * m_lurate);
        }
        //未标记的实例
        ...
        while (e.hasMoreElements()) {
            ...
            //添加到训练已经标记的样本中中去
            //unlabled的数据权重
            instance.setWeight(m_unlabebledWeights);
        ...
        WritableWorkbook workbook = Workbook.createWorkbook(new File("d:\\weightput.xls"));
        WritableSheet sheet = workbook.createSheet("Iris", 0);
        //测试训练数据顺序是否发生变化
        WritableSheet sheetTrain = workbook.createSheet("traindata", 1);
        WritableSheet sheeterrRate=workbook.createSheet("errRate",2);
        //输出预测值
        WritableSheet sheetpre = workbook.createSheet("prediction", 3);

        //==初始类标记 只有一列，放在0列，为便于比较后面的预测标记
               String init = String.valueOf(0);
               Label lab = new Label(0,0, init);
               sheetpre.addCell(lab);
            for (int i = 0; i < trainPool.numInstances(); i++) {
                double lclass= trainPool.instance(i).classValue();
                Number number = new Number(0,i+1,  lclass);
                 sheetpre.addCell(number);
               }
         //===
        for (m_NumIterationsPerformed = 0; m_NumIterationsPerformed < m_Classifiers.length;
            ...
            evalTrainPool.evaluateModel(m_Classifiers[m_NumIterationsPerformed],trainPool,true );
...
//==
            ...
            reweightincorrect = Math.exp(alpha);
             ...
             String it = String.valueOf(m_NumIterationsPerformed);
               Label label = new Label(m_NumIterationsPerformed,0, it);
            sheet.addCell(label);
            for (int i = 0; i < trainPool.numInstances(); i++) {
                double weight = trainPool.instance(i).weight();
                Number number = new Number(m_NumIterationsPerformed,i+1,  weight);
                sheet.addCell(number);
                for(int j=0;j<trainPool.numAttributes();j++)
                {
                    Number attribut = new Number(m_NumIterationsPerformed*trainPool.numAttributes()+j,i,
                            trainPool.instance(i).value(j));
                   sheetTrain.addCell(attribut);
                }
                
            }

           //这个地方应该对分类正确和错误都加权
            setWeights(trainPool, labledDataSet.numInstances(), reweightcorrect, reweightincorrect);
...
//为统一weka的统计信息求法：特意将sums最大的值的的类属概率设置为0.6，其他类属概率平摊
     ...
     */
    // 修改将Labeled 数据中分错的权重加重，为4倍。
    private void setWeights(Instances trainPool, int numberlabeled, double reweightcorrect, double reweightincorrect)
            throws Exception {
        ...
        while (enu.hasMoreElements()) {
            ...
            count++;
            if (!correct) {
                if (count < numberlabeled)//这时候的instance 为labeled的数据
                {
                    instance.setWeight(reweightincorrect * m_luincorrectRate);
                } else {
