...
public class NewMultilayerPerceptron
  ...
  public static void main(String [] argv) {
    runClassifier(new NewMultilayerPerceptron(), argv);
  }
   ...
   * The constructor.
   */
  public NewMultilayerPerceptron() {
    m_instances = null;
...
//    }
      ...
      model.append("Value(Node" + con.getId() + ") = 1.0 * "+ weights[0] +"\n");
      for (int nob = 1; nob < con.getNumInputs() + 1; nob++) {
	if ((inputs[nob - 1].getType() & NeuralConnection.PURE_INPUT)
	    == NeuralConnection.PURE_INPUT) {
	  model.append("    +    Attrib " +
	  ...
	  model.append("    +    Value(Node" + inputs[nob-1].getId() + ")    *    " +
		       weights[nob] + "\n");
	}
      }
      if (con.getMethod() instanceof SigmoidUnit) {
	model.append("Value(Node" + con.getId() + ")  =   Sigmoid( Value(Node"+ con.getId() + ") )\n\n");
      }
      else if (con.getMethod() instanceof LinearUnit) {
	model.append("Value(Node" + con.getId() + ")  =   Linear( Value(Node"+ con.getId() + ") )\n\n");
      }
    }
    //now put in the ends
