...
/**
 <!-- globalinfo-start -->
 * Collective Classifier that uses one classifier (first "-B") for labeling the test data after training on the train set. The second classifier (second "-B") does the actual enhancing of the test set. If more than one classifier follows after the first "-B" then these classifiers are used alternatingly. <br/>
 * The result is then once again tested against the test set.
 * <p/>
 <!-- globalinfo-end -->
 * 
 <!-- options-start -->
 ...
 * @version $Revision$ 
 */
public class TwoStageCollective 
  extends CollectiveRandomizableMultipleClassifiersCombiner 
  implements RestartableCollectiveClassifier, Summarizable {
  
  /** for serialization */
  private static final long serialVersionUID = -2536366962058936976L;
  ...
  /** marks the RMS of the last improvement on on both sets */
  protected double m_LastRMS;
  
  /** marks the accuracy of the last improvement on the train set */
  protected double m_LastAccTrain;
  
  /** the current RMS on the train set (first classifier) */
  protected double m_RMSTrain_1;
  
  /** the current RMS on the test set (first classifier) */
  protected double m_RMSTest_1;
  
  /** the current RMS on the (original) test set (first classifier) */
  protected double m_RMSTestOriginal_1;
  
  /** the current RMS on the both sets (first classifier) */
  protected double m_RMS_1;

  /** the accuracy on the training set (first classifier) */
  protected double m_AccTrain_1;
  
  /** the accuracy on the (original) test set (first classifier) */
  protected double m_AccTestOriginal_1;
  
  /** used for storing the best model */
  protected TwoStageCollective m_BestModel;
  ...
  protected void initializeMembers() {
    ...
    m_RMSTrain_1             = 1.0;
    m_RMSTest_1              = 1.0;
    m_RMSTestOriginal_1      = 1.0;
    m_RMS_1                  = 1.0;
    m_AccTrain_1             = 0.0;
    m_AccTestOriginal_1      = 0.0;
    m_BestModel              = null;
  ...
  public String globalInfo() {
    return    
        "Collective Classifier that uses one classifier (first \"-B\") for " 
      + "labeling the test data after training on the train set. The second "
      + "classifier (second \"-B\") does the actual enhancing of the test set. "
      + "If more than one classifier follows after the first \"-B\" then these "
      + "classifiers are used alternatingly. \n"
      + "The result is then once again tested against the test set." ;
  }
  ...
  public void setOptions(String[] options) throws Exception {
    ...
    if (tmpStr.length() != 0)
      setEvaluationType(
          new SelectedTag(Integer.parseInt(tmpStr), 
            CollectiveInstances.EVAL_TAGS));
    else
      setEvaluationType(
          new SelectedTag(CollectiveInstances.EVAL_RANDOMWALK_LAST, 
            CollectiveInstances.EVAL_TAGS));
    ...
    if (tmpStr.length() != 0)
      ...
      setComparisonType(
          new SelectedTag(CollectiveInstances.COMPARE_RMS, 
            CollectiveInstances.COMPARE_TAGS));
  ...
  public String logTipText() {
    return "Whether to log internal data to tmp directory.";
  }
  
  /**
   * Sets whether to log internal data to tmp directory.
   *
  ...
  public String evaluationTypeTipText() {
    return "The type of evaluation used for evaluating the classifiers.";
  }
   ...
   *
   * @param tag the comparison type.
   */
  public void setComparisonType(SelectedTag tag) {
    if (tag.getTags() == CollectiveInstances.COMPARE_TAGS)
      m_ComparisonType = tag.getSelectedTag().getID();
  }
   ...
   */
  protected TwoStageCollective getBestModel() {
    if (m_BestModel != null)
      return m_BestModel;
    else
      return this;
  ...
  public int compareTo(Object o) {
    int                   result;
    TwoStageCollective    c;
    double                valThis;
    double                valOther;
    
    if (    (o == null) 
        || !(o instanceof CollectiveRandomizableMultipleClassifiersCombiner)) {
      result = 0;
  ...
  public void reset() {
    ...
    m_LogEntries.addFilename(
        "PercMisclassifiedTrain",
        createFilename("-miscls_train.csv"));
    m_LogEntries.addFilename(
        "PercMisclassifiedTestOriginal",
        createFilename("-miscls_test-original.csv"));
    m_LogEntries.addFilename(
        "FlippedLabels",
        createFilename("-flipped.csv"));
    m_LogEntries.addFilename(
        "RMS_1",
        createFilename("-rms_1.csv"));
    m_LogEntries.addFilename(
        "RMSTrain_1",
        createFilename("-rms_train_1.csv"));
    m_LogEntries.addFilename(
        "RMSTest_1",
        createFilename("-rms_test_1.csv"));
    m_LogEntries.addFilename(
        "RMSTestOriginal_1",
        createFilename("-rms_test-original_1.csv"));
    m_LogEntries.addFilename(
        "AccTrain_1",
        createFilename("-acc_train_1.csv"));
    m_LogEntries.addFilename(
        "AccTestOriginal_1",
        createFilename("-acc_test-original_1.csv"));
    m_LogEntries.addFilename(
        "PercMisclassifiedTrain_1",
        createFilename("-miscls_train_1.csv"));
    m_LogEntries.addFilename(
        "PercMisclassifiedTestOriginal_1",
        createFilename("-miscls_test-original_1.csv"));
  }
  ...
  protected double[] getDistribution(Instance instance) throws Exception {
    if (m_EvaluationType == CollectiveInstances.EVAL_RANDOMWALK_LAST) 
      return this.getClassifiers()[1].distributionForInstance(instance);  
    else
      return getBestModel().getClassifiers()[1].distributionForInstance(instance);  
  }
  
  /**
   * returns the run in which the last improvement happened
   * 
   * @return		the last run with improvement
   */
   ...
   * 
   * @return		the iteration with improvement
   */
  ...
  protected void generateSets() throws Exception {
    ...
    m_FlipHistory = new FlipHistory(m_TestsetNew);
  }
  ...
  public void initializeLabels() throws Exception {
    ...
    getClassifiers()[0].buildClassifier(m_Trainset);
    
    // set initial labels
    for (i = 0; i < m_TestsetNew.numInstances(); i++) {
      classification = 
        getClassifiers()[0].classifyInstance(m_TestsetNew.instance(i));
      m_TestsetNew.instance(i).setClassValue(classification);
    }
  }
  ...
  public void flipLabels() throws Exception {
    if (m_EvaluationType == CollectiveInstances.EVAL_HILLCLIMBING)
      m_TestsetNew = m_CollectiveInstances.flipLabels(
                          getBestModel().getCurrentClassifier(), 
                          m_TestsetNew, 0, m_TestsetNew.numInstances(),
                          m_FlipHistory );
    else
      m_TestsetNew = m_CollectiveInstances.flipLabels(
                          this.getCurrentClassifier(), 
                          m_TestsetNew, 0, m_TestsetNew.numInstances(),
                          m_FlipHistory );
  }
  ...
  protected void buildClassifier() throws Exception {
    getNextClassifier().buildClassifier(m_TestsetNew);
  }
  ...
  protected void calculateRMS() throws Exception {
    ...
    rms = CollectiveInstances.calculateRMS(
              ...
              m_Trainset, m_TestsetNew, m_TestsetOriginal);
    ...
    m_RMSTrain_1        = rms[1];
    m_RMSTest_1         = rms[2];
    m_RMSTestOriginal_1 = rms[3];
    
    if (getVerbose())
      System.out.println(   "\nRMSTest_1: "   + m_RMSTest_1 + ", "
                          + "RMSTestOrig_1: " + m_RMSTestOriginal_1 + ", "
                          + "RMSTrain_1: "    + m_RMSTrain_1 + ", "
                          + "RMS_1: "         + m_RMS_1);
  }
  ...
  protected void calculateAccuracy() throws Exception {
    ...
    acc = CollectiveInstances.calculateAccuracy(
            getCurrentClassifier(), m_Trainset, m_TestsetOriginal);
    ...
    acc = CollectiveInstances.calculateAccuracy(
            getClassifiers()[0], m_Trainset, m_TestsetOriginal);

    m_AccTrain_1        = acc[0];
    m_AccTestOriginal_1 = acc[1];
    
    if (getVerbose())
      System.out.println(   "\nAccTrain_1: "   + m_AccTrain_1 + ", "
                          + "AccTestOrig_1: " + m_AccTestOriginal_1);
  }
      ...
      for (i = 0; i < m_NumIterations; i++) {
        ...
        if (    (m_BestModel == null)
          ...
          if (    (m_EvaluationType == CollectiveInstances.EVAL_RANDOMWALK_BEST)
            ...
            m_BestModel = (TwoStageCollective) makeCopy(this);
        }
  ...
  protected void logQuality(int restart, int iteration)
    ...
    m_LogEntries.addValue("PercMisclassifiedTrain",          1 - m_AccTrain);
    m_LogEntries.addValue("PercMisclassifiedTestOriginal",   1 - m_AccTestOriginal);
    m_LogEntries.addValue("FlippedLabels",     getFlippedLabels());
    m_LogEntries.addValue("RMS_1",             m_RMS_1);
    m_LogEntries.addValue("RMSTrain_1",        m_RMSTrain_1);
    m_LogEntries.addValue("RMSTest_1",         m_RMSTest_1);
    m_LogEntries.addValue("RMSTestOriginal_1", m_RMSTestOriginal_1);
    m_LogEntries.addValue("AccTrain_1",        m_AccTrain_1);
    m_LogEntries.addValue("AccTestOriginal_1", m_AccTestOriginal_1);
    m_LogEntries.addValue("PercMisclassifiedTrain_1",        1 - m_AccTrain_1);
    m_LogEntries.addValue("PercMisclassifiedTestOriginal_1", 1 - m_AccTestOriginal_1);
  ...
  public static void main(String[] args) {
    runClassifier(new TwoStageCollective(), args);
  }
}
