...
public class LLGC
    ...
    public String globalInfo() {
        return "A collective classifier that generates a smooth classifying function " + "for labeled and unlabeled data. It pre-processes the data with the " + "following filters:\n" + " - weka.filters.unsupervised.attribute.ReplaceMissingValues\n" + " - weka.filters.unsupervised.attribute.NominalToBinary\n" + " - if -N 0 then weka.filters.unsupervised.attribute.Normalize\n" + " - if -N 1 then weka.filters.unsupervised.attribute.Standardize\n" + "\n" + "For more informations, refer to the following paper:\n\n" + getTechnicalInformation().toString() + "\n\n" + "The following modification was done (option -include-atts):\n" + "- the distance between two instances is not divided by " + "2*sigma^2, but by 2*sigm^2*N, with N as the number of" + "attributes.";
    }
    ...
    public String includeNumAttributesTipText() {
        return "Whether to include the number of attributes in the calculation " + "of the affinity matrix (default yes).";
    }
...
//利用父类的成员变量
    ...
    protected void initialize() throws Exception {
        ...
        for (i = 0; i < numInst; i++) {
            for (n = 0; n < numInst; n++) {
                if (i == n) {
                    ...
                    d = StrictMath.exp(
                            -StrictMath.pow(d, 2) / (2 * getSigma() * getSigma() * factor));
                }
        ...
        switch (m_SequenceLimit) {
            ...
            default:
                throw new Exception("Unknown sequence limit function: " + m_SequenceLimit + "!");
        }
        clock("Matrix S/P");
    }
    ...
    protected String toStringParameters() {
        ...
        result += "Sequence Limit Func...: " + m_SequenceLimit + " (" + getSequenceLimit().getSelectedTag().getReadable() + ")\n";
