 ...
 * @see OptionHandler
 */
public class MPCKMeans extends AbstractClusterer implements  OptionHandler, SemiSupClusterer {
    ...
    int assignPoints() throws Exception {
        ...
        if (m_verbose) {
            ...
            System.out.println("\tvar=" + ((float) m_objVariance) + "\tC=" + ((float) m_objCannotLinks) + "\tM=" + ((float) m_objMustLinks) + "\tLOG=" + ((float) m_objNormalizer) + "\tREG=" + ((float) m_objRegularizer));
        }
    ...
    public int assignInstanceToClusterWithConstraints(int instIdx) throws Exception {
        ...
        if (m_ClusterAssignments[instIdx] != bestCluster) {
            if (m_ClusterAssignments[instIdx] >= 0 && m_ClusterAssignments[instIdx] < m_NumClusters) {
                //if (m_verbose) {
                System.out.println("Moving instance " + instIdx + " from cluster " + m_ClusterAssignments[instIdx] + " to cluster " + bestCluster + " penalty:" + ((float) penaltyForInstance(instIdx, m_ClusterAssignments[instIdx])) + "=>" + ((float) lowestPenalty));
            }
            moved = 1;
    ...
    public double penaltyForInstance(int instIdx, int centroidIdx) throws Exception {
        ...
        double total = m_objVarianceCurrPoint + m_objCannotLinksCurrPoint + m_objMustLinksCurrPoint + m_objNormalizerCurrPoint;
        if (m_verbose) {
            System.out.println("Final penalty for instance " + instIdx + " and centroid " + centroidIdx + " is: " + total);
        }
        return total;
    }
...
//        }
    ...
    public double calculateObjectiveFunction(boolean isComplete) throws Exception {
        ...
        if ((float) m_OldObjective < (float) m_Objective) {
            ...
            System.out.println("Old objective:" + (float) m_OldObjective + " < New objective: " + (float) m_Objective);
        }
            ...
            if (dataset.equals("lowd")) {
                ...
                while (num < numPairs) {
                    ...
                    int linkType = (data.instance(first).classValue() ==
                            data.instance(second).classValue()) ? InstancePair.MUST_LINK : InstancePair.CANNOT_LINK;
