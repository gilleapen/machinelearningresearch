...
public class MinSubsetFisherAttributeEval
     ...
     * Constructor
     */
    public MinSubsetFisherAttributeEval() {
        resetOptions();
    }
    ...
    public double evaluateSubset(BitSet subset) throws Exception {
        ...
        for (int cl = 0; cl < m_numClasses; cl++) {
            ...
            double ratio = diffClassDiff / sameClassDiff;
            weight = weight * ratio;
            double temp = Math.abs(ratio);
            if (temp > max) {
                max = temp;
            }
        }
        weight = weight * max / subset.size();
        weight = Math.pow(weight, 1.0 / m_numClasses);
