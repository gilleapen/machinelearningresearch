 ...
 * (clusterers and classifiers), where labeled data is not present for
 * all the categories
 *
 * @author Sugato Basu */

public class  SemiSupIncompleteLabelCurveCVResultProducer 
  ...
  /** 
   * The minimum number of labeled categories to drop. If this is zero, the first
   * step will drop m_StepSize categories
   */
  protected int m_LowerSize = 0;
  
  /**
   * The maximum number of labeled categories to drop. -1 indicates no maximum 
   * (other than the total number of categories)
   */
  protected int m_UpperSize = -1;

  /** The number of labeled categories to drop at each step */
  protected int m_StepSize = 2;

  /** 
   * The specific points to plot, integers representing specific
   * numbers of incomplete labels 
   */
  ...
  protected int m_NumMissingLabels = 0;
  ...
  public void setAdditionalMeasures(String [] additionalMeasures) {
    ...
    if (m_SplitEvaluator != null) {
      System.err.println(" SemiSupIncompleteLabelCurveCVResultProducer: setting additional "
			 +"measures for "
			 +"split evaluator");
  ...
  public double getMeasure(String additionalMeasureName) {
	...
	getMeasure(additionalMeasureName);
      ...
      throw new IllegalArgumentException(" SemiSupIncompleteLabelCurveCVResultProducer: "
  ...
  public void doRunKeys(int run) throws Exception {
	...
	m_NumMissingLabels = plotPoint(0);
      }
      else if (m_LowerSize == 0) {
	m_NumMissingLabels = m_StepSize;
      } else {
	m_NumMissingLabels = m_LowerSize;
      }
      while (m_NumMissingLabels < m_Instances.numClasses()) {
	...
	if (m_PlotPoints != null) {
	  pointNum ++;
	  m_NumMissingLabels = plotPoint(pointNum);
	}
	else {
	  m_NumMissingLabels += m_StepSize;
	}
  ...
  public void doRun(int run) throws Exception {
    int numExtraKeys;
    int numClasses = m_Instances.numClasses();
    ...
    for (int fold = 0; fold < m_NumFolds; fold++) {
      ...
      Instances testSet = runInstances.testCV(m_NumFolds, fold);

      /*
      for (int i=0; i<train.numInstances(); i++) {
	System.out.println("Train instance has class: " + train.instance(i).classValue());
      }
      for (int i=0; i<testSet.numInstances(); i++) {
	System.out.println("Test instance has class: " + testSet.instance(i).classValue());
      }
      */
	...
	m_NumMissingLabels = m_LowerSize;
      ...
      while (m_NumMissingLabels < numClasses) {
	// Add in some fields to the key like run and fold number, dataset name
	Object [] seKey = m_SplitEvaluator.getKey();
	Object [] key = new Object [seKey.length + numExtraKeys];
	key[0] = Utils.backQuoteChars(m_Instances.relationName());
	...
	if (m_ResultListener.isResultRequired(this, key)) {
	  try {
	    if(m_IsFraction)
	      System.out.println("Run:" + run + " Fold:" + fold + " Size:" + m_CurrentSize + " Fraction:" + m_PlotPoints[pointNum] + " NumMissingLablels:" + m_NumMissingLabels);
	    else 
	      System.out.println("Run:" + run + " Fold:" + fold + " Size:" + m_CurrentSize  + " NumMissingLablels:" + m_NumMissingLabels);
	    ...
	    // Select random set of missing class labels to remove from training data 
	    Random random = new Random(fold);
	    int [] marks = new int[numClasses];
	    for (int i=0; i<numClasses; i++) 
	      marks[i] = 0;
	    for (int rand=0; rand<m_NumMissingLabels; rand++) {
	      int classToRemove = random.nextInt(numClasses);
	      while (marks[classToRemove] == 1) {
		classToRemove = random.nextInt(numClasses);
	      }
	      marks[classToRemove] = 1;
	    }
	    
	    // Remove labeled data with missing class labels from training set
	    /*
	    for (int i=0; i<numClasses; i++) {
	      if (marks[i] == 1)
		System.out.println("Removing class: " + i + " from train");
	      else
		System.out.println("Keeping class: " + i + " in train");
	    }
	    */

	    for (int num=0; num<labeledTrainSubset.numInstances(); num++) {
	      Instance inst = labeledTrainSubset.instance(num);
	      if (marks[(int) inst.classValue()] == 1) {
		//		System.out.println("Removing train instance with class " + inst.classValue());
		inst.setClassMissing();
	      }
	    }
	    labeledTrainSubset.deleteWithMissingClass();
	    System.out.println("NumInstances in train: " + labeledTrainSubset.numInstances());

	    // Keep data only having missing class labels in test set
	    
	    if (m_NumMissingLabels != 0) { // if no classes removed, do normal clustering
	      for (int num=0; num<test.numInstances(); num++) {
		Instance inst = test.instance(num);
		if (marks[(int) inst.classValue()] == 0) {
		  //		  System.out.println("Removing test instance with class " + inst.classValue());
		  inst.setClassMissing();
		}
	      }
	      test.deleteWithMissingClass();
	    }
	    System.out.println("NumInstances in test: " + test.numInstances());
	    ...
	    if (m_SplitEvaluator instanceof SemiSupClustererSplitEvaluator) {
	      seResults = ((SemiSupClustererSplitEvaluator) m_SplitEvaluator).getResult(labeledTrainSubset, unlabeledTrainSubset, test, numClasses); 
	    }
	    else {
  ...
  protected int plotPoint(int i) {
    ...
    int length = m_PlotPoints.length;  
    double point = 0;
    if (i >= length) {
      point = m_Instances.numClasses() + 1;
    }
    else {
      point = m_PlotPoints[i];
    }
    return (int)point;
  }
  ...
  public String lowerSizeTipText() {
    return "Set the minimum number of categories to drop in a training set. Setting zero "
      + "here will actually drop <stepSize> number of categories at the first "
      + "step ";
  }
  ...
  public String upperSizeTipText() {
    return "Set the maximum number of labeled categories to drop in a training set. Setting -1 "
      + "sets no upper limit (other than the total number of categories "
  ...
  public String stepSizeTipText() {
    return "Set the number of labeled categories to drop from the training data at each step.";
  }
  ...
  public String plotPointsTipText() {
    return "A list of specific points to plot as a string of numbers separated by commas or spaces. "+
      "Whole numbers indicate a specific number of categories to drop "+
  ...
  public String toString() {
    ...
    String result = "SemiSupIncompleteLabelCurveCVResultProducer: ";
  ...
  public Instances interleave (Instances train, int numClasses) {
    int numInstances = train.numInstances();
    int step = (int) Math.ceil(numInstances/numClasses);
    Instances interleavedInst = new Instances(train, numInstances);
    
    //    System.out.println("Step: " + step + ", numInstances: " + numInstances + ", numClasses: " + numClasses);
    for (int j=0; j<step; j++) {
      for (int i=0; i<numClasses; i++) {
	if (j + step*i < numInstances) {
	  interleavedInst.add(train.instance(j + step*i));
	  //	  System.out.println("Added instance " + (j+step*i) + " with class: " + train.instance(j + step*i).classValue());
	}
      }
    }
    return interleavedInst;
  }
...
} //  SemiSupIncompleteLabelCurveCVResultProducer
